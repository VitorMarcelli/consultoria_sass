...
export default function OportunidadesPage() {
  ...
  const fetchOpportunities = async () => {
    try {
      const data = await apiRequest('/opportunities').catch(() => []);
  ...
  const handleOpenCreateModal = async () => {
    setIsModalOpen(true);
    try {
      const c = await apiRequest('/clients').catch(() => []);
      setClients(c);
      if (c.length) {
  ...
  const handleTriggerScan = async () => {
    ...
    try {
      ...
      alert(`Scanner de BI Executado com Sucesso! Foram identificadas ${res.generatedCount} novas oportunidades automáticas de Upsell / Negociação na base.`);
      fetchOpportunities();
    } catch (err: any) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-3">
            <Zap className="h-8 w-8 text-teal-500 fill-teal-500/20" />
            Painel de Oportunidades & BI
          </h1>
          <p className="text-slate-500 font-medium mt-1">
            Ferramenta Interna TASS para mineração de upsell, estouro de horas e inteligência comercial.
          </p>
        </div>
      ...
      {/* Hero Banner de Metricas BI */}
      <div className="bg-gradient-to-tr from-slate-900 via-slate-850 to-slate-950 rounded-[2.5rem] p-8 text-white shadow-2xl relative overflow-hidden border border-slate-800">
        <div className="absolute top-0 right-0 w-2/3 h-full bg-gradient-to-l from-emerald-500/10 via-transparent to-transparent pointer-events-none blur-3xl"></div>
        
        <div className="relative z-10 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8">
          <div className="flex flex-col gap-2 max-w-xl">
            <span className="text-[10px] font-black text-emerald-400 uppercase tracking-[0.2em] bg-emerald-500/10 px-3 py-1 rounded-full w-max border border-emerald-500/20 flex items-center gap-1">
            ...
            <p className="text-slate-400 text-sm font-medium leading-relaxed">
              O motor examina contratos que ultrapassaram a franquia de horas no timesheet, picos de processamento fiscal e rotatividade de departamento pessoal para propor revisões de honorários.
            </p>
          </div>

          <div className="flex flex-wrap sm:flex-nowrap gap-4 w-full lg:w-auto">
            <div className="flex-1 sm:flex-none p-5 bg-slate-800/80 backdrop-blur-xl border border-slate-700/60 rounded-3xl flex flex-col items-center text-center min-w-[150px]">
              <span className="text-3xl font-black text-emerald-400 tracking-tight">R$ {wonValue.toFixed(2)}</span>
              <span className="text-xs font-bold text-slate-400 uppercase tracking-wider mt-1">Novas Receitas (Ganhas)</span>
            </div>
            <div className="flex-1 sm:flex-none p-5 bg-slate-800/80 backdrop-blur-xl border border-slate-700/60 rounded-3xl flex flex-col items-center text-center min-w-[150px]">
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <input required type="text" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} className="w-full h-11 rounded-2xl border border-slate-200 dark:border-slate-800 px-4 text-sm font-medium outline-none focus:border-teal-500 transition-all bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white" placeholder="Ex: Aditivo Contratual - Folha de Pagamento" />
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input required type="number" step="0.01" value={formData.estimatedValue} onChange={e => setFormData({...formData, estimatedValue: Number(e.target.value)})} className="w-full h-11 rounded-2xl border border-slate-200 dark:border-slate-800 px-4 text-sm font-medium outline-none focus:border-teal-500 transition-all bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white" placeholder="Ex: 2500.00" />
                  </div>
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">Descrição / Motivo</label>
                  <textarea required value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} className="w-full h-24 rounded-2xl border border-slate-200 dark:border-slate-800 p-4 text-sm font-medium outline-none focus:border-teal-500 transition-all bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white resize-none" placeholder="Informe os detalhes observados na operação..." />
                </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-sm text-center px-4">
          ...
          <p className="text-slate-500 font-medium mt-2 max-w-sm">
            Clique no botão "Scanner Automático de BI" acima para buscar anomalias operacionais e sugerir revisões de honorários automaticamente.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredOpportunities.map((op) => {
            ...
            return (
              <div 
                key={op.id}
                className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[2rem] p-6 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col justify-between group"
              >
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${
                      op.status === 'GANHA' ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' :
                      op.status === 'PERDIDA' ? 'bg-slate-100 dark:bg-slate-800 text-slate-500' :
                  ...
                  <div>
                    ...
                    <h3 className="text-lg font-extrabold text-slate-900 dark:text-white tracking-tight group-hover:text-teal-600 dark:group-hover:text-teal-400 transition-colors">
                      {op.title}
                    </h3>
                  </div>
                  ...
                  <div className="flex items-baseline justify-between pt-2 border-t border-slate-100 dark:border-slate-800">
                    <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Honorário / Receita Extra</span>
                ...
                <div className="flex items-center justify-between gap-2 mt-6 pt-4 border-t border-slate-100 dark:border-slate-800">
                    ...
                    <>
                      ...
                      {op.status === 'IDENTIFICADA' && (
                        <button 
                          ...
                          className="flex-1 py-2.5 bg-teal-50 hover:bg-teal-600 text-teal-700 hover:text-white dark:bg-teal-500/10 dark:text-teal-400 dark:hover:bg-teal-600 dark:hover:text-white text-xs font-extrabold rounded-xl transition-colors flex items-center justify-center gap-1"
                        >
                          Negociar
                  ...
                  {op.status === 'PERDIDA' && (
                    <div className="w-full py-2.5 bg-slate-100 dark:bg-slate-800 text-slate-500 text-xs font-extrabold rounded-xl text-center">
