  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col lg:flex-row gap-8 items-start">
        {/* Sidebar Nav */}
        <div className="w-full lg:w-72 shrink-0 space-y-1.5 bg-white dark:bg-slate-900 rounded-container border border-slate-100 dark:border-slate-800 p-4 shadow-sm transition-colors">
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-inner text-left transition-all ${
                  isActive 
                    ? 'bg-teal-600 text-white shadow-md shadow-teal-600/20' 
                    : 'text-slate-600 dark:text-slate-400 hover:bg-teal-50 dark:hover:bg-slate-800 hover:text-teal-900 dark:hover:text-slate-200'
                }`}
              >
                <tab.icon className={`h-5 w-5 ${isActive ? 'text-teal-200' : 'text-slate-400 dark:text-slate-500'}`} />
                <div>
                  <div className="text-sm font-bold tracking-wide">{tab.name}</div>
                  <div className={`text-[10px] font-bold uppercase tracking-wider mt-0.5 ${isActive ? 'text-teal-200/80' : 'text-slate-400 dark:text-slate-500'}`}>
                    {tab.desc}
                  </div>
                </div>
              </button>
