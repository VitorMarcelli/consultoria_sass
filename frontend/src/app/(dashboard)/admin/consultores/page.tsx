...
export default function ConsultoresPage() {
  ...
  const [novoRole, setNovoRole] = useState('CONSULTANT');
  ...
  const handleSaveConsultor = async (e: React.FormEvent) => {
    ...
    try {
      if (selectedConsultor) {
        await apiRequest(`/users/${selectedConsultor.id}/role`, {
          method: 'PATCH',
          body: JSON.stringify({ role: novoRole })
        }).catch(() => console.warn('Patch role endpoint might fail se não estiver implementado. Simulando...'));
        
        setConsultores(consultores.map(c => 
          c.id === selectedConsultor.id ? { ...c, name: novoNome, email: novoEmail, role: novoRole } : c
        ));
      } else {
        ...
        alert('Nota: A criação real via Supabase Auth requer envio de convite, isto é uma simulação no UI.');
      }
    ...
    <div className="relative pb-20">
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-3xl font-black text-slate-900 tracking-tight">Equipe Global</h2>
            <p className="text-sm text-slate-500 font-medium mt-1">Gestão de acessos, administradores e consultores.</p>
          </div>
        </div>
        <button 
          onClick={openNewConsultor}
          className="flex items-center gap-2 bg-slate-900 text-white px-6 py-3 rounded-2xl hover:bg-indigo-600 transition-all font-bold text-sm shadow-xl hover:shadow-indigo-600/30 hover:-translate-y-0.5"
        >
        ...
        <div className="p-6 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row gap-4 justify-between items-center">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 px-4 py-2 bg-white rounded-xl border border-slate-200 shadow-sm">
              ...
              <span className="text-xs font-bold text-slate-600">{consultores.filter(c => c.status === 'ACTIVE').length} Ativos</span>
            </div>
            <div className="flex items-center gap-2 px-4 py-2 bg-white rounded-xl border border-slate-200 shadow-sm">
              <Shield className="w-3.5 h-3.5 text-indigo-500" />
              <span className="text-xs font-bold text-slate-600">{consultores.filter(c => c.role === 'ADMIN').length} Admins</span>
            </div>
          </div>
        </div>
            ...
            >
              <div className="flex items-center justify-between px-8 py-6 border-b border-slate-100/50">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-xl font-black text-slate-900">{selectedConsultor ? 'Editar Membro' : 'Convidar Membro'}</h2>
                    <p className="text-xs font-semibold text-slate-500">Gestão de acesso ao sistema</p>
                  </div>
                </div>
              ...
              <form onSubmit={handleSaveConsultor} className="p-8 space-y-6">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider ml-1">Nível de Acesso (Cargo)</label>
                  <div className="relative">
                    <select 
                      value={novoRole} 
                      onChange={(e) => setNovoRole(e.target.value)} 
                      className="w-full pl-10 pr-5 py-3.5 bg-white/50 border border-slate-200 rounded-2xl text-sm font-bold focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all hover:border-slate-300 appearance-none text-slate-700"
                    >
                      <option value="CONSULTANT">Consultor de Operação</option>
                      <option value="ADMIN">Administrador Global</option>
                    </select>
                    {novoRole === 'ADMIN' ? (
                      <Shield className="w-4 h-4 text-indigo-500 absolute left-4 top-1/2 -translate-y-1/2 pointer-events-none" />
                    ) : (
                  ...
                  <p className="text-[11px] font-medium text-slate-500 mt-2 px-1">
                    {novoRole === 'ADMIN' ? 'Administradores têm acesso total às configurações e escritórios.' : 'Consultores acessam apenas as áreas e clientes alocados a eles.'}
                  </p>
                </div>
