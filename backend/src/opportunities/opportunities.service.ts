...
export class OpportunitiesService {
    ...
    return tenantPrisma.opportunity.create({
      data: {
        ...
        potentialValue: data.potentialValue
          ? parseFloat(data.potentialValue)
          : null,
        status: data.status || 'IDENTIFIED',
        observations: data.observations,
        clientPortalSummary:
          data.clientPortalSummary ||
          'Análise de ganho de eficiência operacional e financeira.',
      },
      include: { client: true },
    });
  }
  ...
  async handleDailyScan() {
    ...
    for (const tenant of tenants) {
      try {
        ...
        this.logger.error(
          `Erro ao gerar oportunidades para tenant ${tenant.id}`,
          err,
        );
      }
    }
  ...
  async scanAndGenerateOpportunities(tenantId: string) {
    ...
    for (const client of clients) {
      const totalHours =
        client.timeLogs.reduce(
          (acc, log) => acc + (log.durationMinutes || 0),
          0,
        ) / 60;
      const totalCost = client.timeLogs.reduce(
      ...
      for (const front of client.frontClassifications) {
        ...
        if (front.taxInfo?.monthlyNotesVolume)
          taxVolume +=
            parseInt(front.taxInfo.monthlyNotesVolume.replace(/\D/g, ''), 10) ||
            0;
      }
      ...
      if (apiKey) {
        try {
          ...
          this.logger.error(
            `Erro ao gerar oportunidade via IA para o cliente ${client.id}:`,
            err,
          );
        }
      } else {
          ...
          if (!exists) {
            await tenantPrisma.opportunity.create({
              data: {
                ...
                clientPortalSummary:
                  'Estudo executivo para otimização de escopo e garantia de equilíbrio operacional no atendimento.',
              },
            });
            generatedCount++;
          }
  ...
  async getDashboardData(tenantId: string) {
    ...
    const totalCosts = recentTimeLogs.reduce(
      (acc, log) => acc + (log.costAmount || 0),
      0,
    );
    ...
    recentTimeLogsForHealth.forEach((log) => {
      if (log.clientId) {
        clientCostsMap[log.clientId] =
          (clientCostsMap[log.clientId] || 0) + (log.costAmount || 0);
      }
    });
    ...
    const recentActivityWithHealth = opps.map((o) => {
      ...
      if (categoriesMap[o.category] !== undefined) {
        categoriesMap[o.category]++;
      } else {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const monthRevenue = clientsBeforeEnd.reduce(
        (acc, c) => acc + (c.monthlyFee || 0),
        0,
      );
...
O e-mail deve ser amigável, propondo uma reunião para apresentar a solução. Assine como 'Equipe Sevilha Contabilidade'. Não use placeholders com colchetes, gere pronto para envio.`;
      ...
      return {
        emailBody: 'Erro ao conectar com a IA. Tente novamente mais tarde.',
      };
    }
  }
}
