...
async function bootstrap() {
  ...
  console.log(
    `[NETWORK] Backend server is running on port ${port} (restarted)`,
  );
  console.log(
    `[NETWORK] Server physical binding address: ${JSON.stringify(address)}`,
  );
  ...
  setInterval(() => {
    require('http')
      .get(`http://127.0.0.1:${port}/`, (res: any) => {
        console.log(
          `[INTERNAL PING] Health check to itself: HTTP ${res.statusCode}`,
        );
      })
      .on('error', (err: any) => {
